(function () {
  ...
  function ensureMemberDefaults(r){
    return {
      id:r.id||Date.now(),
      username:r.username||"",
      password:r.password||"",
      ...
      role:r.role||"member",
      fullName:r.fullName||r.username||"Üye",
      company:r.company||"",
  ...
  function registerMember(p){
    ...
    const row=ensureMemberDefaults({
      ...
      provider:String(p.provider||"manual").trim()||"manual",
      role:String(p.role||"member").trim()||"member",
      fullName:String(p.fullName||"").trim()||u,
  ...
  function findMember(u,pw){
    u=String(u||"").trim(); pw=String(pw||"").trim();
    return getMembers().find(x=>String(x.username)===u&&String(x.password)===pw&&String(x.status)!=="Pasif")||null;
  }
  function findMemberByPhone(phone){
    const n=String(phone||"").replace(/\s+/g,"");
    return getMembers().find(x=>String(x.phone||"").replace(/\s+/g,"")===n&&String(x.status)!=="Pasif")||null;
  }
  function quickPhoneLogin(p){
    ...
    if(!row){
      ...
      row=ensureMemberDefaults({
        ...
        provider:"phone",
        role:String(p.role||"member").trim()||"member",
        fullName:String(p.fullName||"Telefon Üyesi").trim(),
        company:String(p.company||"").trim(),
        status:"Aktif",
        createdAt:new Date().toISOString()
      });
  ...
  function socialLoginMock(provider,p){
    const sp=String(provider||"social").trim().toLowerCase();
    const fn=String(p.fullName||"").trim()||(sp==="google"?"Google Üyesi":"Facebook Üyesi");
    const role=String(p.role||"member").trim()||"member";
    const company=String(p.company||"").trim();
    const username=sp+"_"+fn.toLowerCase().replace(/[^a-z0-9]+/g,"_");
    const m=getMembers();
    let row=m.find(x=>x.username===username&&x.provider===sp);
    if(!row){
      row=ensureMemberDefaults({
        id:Date.now(), username:username, password:sp+"-login", phone:"",
        provider:sp, role:role, fullName:fn, company:company, status:"Aktif",
        createdAt:new Date().toISOString()
      });
      m.push(row); saveMembers(m);
    }
  ...
  function setMemberStatus(id,status){
    const r=getMembers(),row=r.find(x=>String(x.id)===String(id));
    if(!row) throw new Error("Üye bulunamadı.");
    row.status=status; saveMembers(r); return row;
  }
  ...
  function setApplicationStatus(id,status){
    const r=getApplications(),row=r.find(x=>String(x.id)===String(id));
    if(!row) throw new Error("Başvuru bulunamadı.");
    row.status=status; saveApplications(r); return row;
  }
  ...
  function setProductStatus(id,status){
    const r=getProducts(),row=r.find(x=>String(x.id)===String(id));
    if(!row) throw new Error("Ürün bulunamadı.");
    row.status=status; saveProducts(r); return row;
  }
  ...
  function addMessage(p){
    const r=getMessages();
    const row={id:Date.now(),from:String(p.from||"").trim()||"Bilinmeyen",subject:String(p.subject||"").trim()||"Konu yok",message:String(p.message||"").trim()||"-",status:"Yeni",createdAt:new Date().toISOString()};
  ...
  function setMessageStatus(id,status){
    const r=getMessages(),row=r.find(x=>String(x.id)===String(id));
    if(!row) throw new Error("Mesaj bulunamadı.");
    row.status=status; saveMessages(r); return row;
  }
  ...
  function trackVisit(page){
    const r=getVisits();
    r.push({id:Date.now()+Math.random(),page:String(page||"unknown"),time:new Date().toISOString(),user:getAuth()?.username||"ziyaretci"});
    saveVisits(r);
  }
  function getVisitSummary(){
    const r=getVisits(),byPage={};
    r.forEach(x=>{const k=x.page||"unknown";byPage[k]=(byPage[k]||0)+1});
    return {total:r.length,byPage:byPage,recent:r.slice(-20).reverse()};
  }
  ...
  function setLoadStatus(id,status){
    const r=getLoads(),row=r.find(x=>String(x.id)===String(id));
    if(!row) throw new Error("İlan bulunamadı.");
    row.status=status; saveLoads(r); return row;
  }

  function getOffers(){
    const r=parse(OFFERS_KEY,[]);
    return Array.isArray(r)?r.map(x=>({...x,adminNote:x.adminNote||"",updatedAt:x.updatedAt||x.createdAt||new Date().toISOString()})):[];
  }
  ...
  function setOfferStatus(id,status){
    ...
    row.status=status; row.updatedAt=new Date().toISOString(); saveOffers(r); return row;
  }
